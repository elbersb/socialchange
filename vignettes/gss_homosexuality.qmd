...
```{r}
# decompose_aggregated() requires every wave to share a common minimum and
# maximum age. The 2014 and 2016 waves sampled no one under 19 and 21
# respectively, so we restrict to age >= 21; the earliest waves have no
# respondents past 81, so we top-code age at 81. The shared maximum age is
# treated as an open interval ("81+"): survivors reaching it stay there instead
# of aging out of the observed range.
...
# pool the frame's counts at 81 to match the survey's open interval ("81+")
pop <- pop[, .(n = sum(n)), by = .(period, age = pmin(age, 81), sex)]
