...
If we continue to ignore migration, the only other input that is required is a function that predicts the outcome for a given cell. This function can be as complex or simple as needed, it just needs to fulfill a number of requirements:

- Takes as inputs the defined cells (such as age and gender), plus current period. It is not necessary that these are all used for the model's predictions, though.
