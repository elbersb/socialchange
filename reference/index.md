# Package index

## All functions

- [`apc()`](https://elbersb.github.io/socialchange/reference/apc.md) :
  Age-Period-Cohort model estimation
- [`apc_nonlinearities()`](https://elbersb.github.io/socialchange/reference/apc_nonlinearities.md)
  : Extract non-linear APC effects
- [`apc_plot_nonlinearities()`](https://elbersb.github.io/socialchange/reference/apc_plot_nonlinearities.md)
  : Plot non-linear APC effects
- [`apc_plot_total()`](https://elbersb.github.io/socialchange/reference/apc_plot_total.md)
  : Plot total APC effects under assumption
- [`apc_plot_two2d()`](https://elbersb.github.io/socialchange/reference/apc_plot_two2d.md)
  : Plot APC two-dimensional visualization
- [`apc_total()`](https://elbersb.github.io/socialchange/reference/apc_total.md)
  : Compute total APC effects under assumption
- [`cr_ic()`](https://elbersb.github.io/socialchange/reference/cr_ic.md)
  : Cohort replacement vs. intracohort change decomposition
- [`decompose_aggregated()`](https://elbersb.github.io/socialchange/reference/decompose_aggregated.md)
  [`plot(`*`<social_change_decomp>`*`)`](https://elbersb.github.io/socialchange/reference/decompose_aggregated.md)
  : Decompose social change from aggregated data
- [`decompose_events()`](https://elbersb.github.io/socialchange/reference/decompose_events.md)
  : Decompose social change from discrete events
- [`eu_membership`](https://elbersb.github.io/socialchange/reference/eu_membership.md)
  : EU membership events
- [`gss_homosex`](https://elbersb.github.io/socialchange/reference/gss_homosex.md)
  : GSS attitudes toward homosexual sex relations
- [`gss_rac`](https://elbersb.github.io/socialchange/reference/gss_rac.md)
  : GSS racial attitudes of white Americans
- [`plot(`*`<cr_ic_decomposition>`*`)`](https://elbersb.github.io/socialchange/reference/plot.cr_ic_decomposition.md)
  : Plot a CR/IC decomposition
- [`plot(`*`<decompose_events>`*`)`](https://elbersb.github.io/socialchange/reference/plot.decompose_events.md)
  : Plot a decompose_events object
- [`plot_gam_surface()`](https://elbersb.github.io/socialchange/reference/plot_gam_surface.md)
  : Contour plot of a 2D GAM smooth surface
- [`print(`*`<apc_model>`*`)`](https://elbersb.github.io/socialchange/reference/print.apc_model.md)
  : Print an APC model
- [`print(`*`<decompose_events>`*`)`](https://elbersb.github.io/socialchange/reference/print.decompose_events.md)
  : Print a decompose_events object
- [`print(`*`<social_change_decomp>`*`)`](https://elbersb.github.io/socialchange/reference/print.social_change_decomp.md)
  : Print a social_change_decomp object
- [`sim_social_change()`](https://elbersb.github.io/socialchange/reference/sim_social_change.md)
  : Simulate social change with demographic processes
- [`wpp_data`](https://elbersb.github.io/socialchange/reference/wpp_data.md)
  : UN World Population Prospects population data
- [`wpp_us`](https://elbersb.github.io/socialchange/reference/wpp_us.md)
  : US population by age and sex, 1973–2016
